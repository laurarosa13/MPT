	...
	io.on('connection', function (socket) {
		...
		socket.on('disconnect', function () {
			...
			if (socket.handshake.address == jugadores.jugador2.ip) {
				...
				resultados = [];
				jugadores = {};
				//jugadores.jugador1.contador = 0;
				//jugadores.jugador1.mazo = [];
				//indice = 0;
				//respuestaCorrecta = "";
				//respuestaAuxiliar = {};
				//ultimoGanador = "";
...
process.on('SIGTERM', function() {
	//jugadores['jugador1'] = { nombre: jugadores.jugador2.nombre, ip: jugadores.jugador2.ip, contador: 0, mazo: [] };
	//var resultados = [];
	//PROBAR HACIENDO GLOBALES LAS OTRAS VARIABLES Y REINICIANDOLAS
	//EL ERROR SE DA CUANDO UNO QUE FUE CLIENTE INTENTA SER SERVIDOR
	console.log('Saliendo.');
	io.emit('serveroff', jugadores.jugador2.ip);
	process.exit(0);
});
